...
- **Consequences:**
  - **Code aligned 2026-08-02 (Session 12).** `AcquisitionPolicy` is now a single `authorized` boolean; `assert_policy` checks only that flag; `investigation.authorize/is_authorized` and the `authorize` CLI verb are gone; `analyze`/`tui`/`investigate`/`step` share one `--authorized` flag. (The original records-level decision was 2026-08-02 / Session 11; `src/` enforced the per-technique model until Session 12.)
...
--
## ADR-19: Session replay classification — three categories, each decided independently (2026-08-02)
- **Status:** proposed (drafted by Session 14 / Buffy, awaiting user acceptance)
- **Context:** Session 14 refined the kickoff feature list and surfaced "session replay — capture and replay portal session tokens" as a real backlog candidate. ADR-18 already sets the binding pattern: "probe methods never authenticate, submit credentials, send exploit payloads, or attempt to obtain access." Replay is qualitatively different from the existing CONNECT / DNS / click-through probes because some replay shapes attempt to obtain access (using a captured credential); other shapes are constructive protocol testing (a synthetic token echoed back to the portal to observe validation behavior). A blanket approval behind `--authorized` is too coarse; ADR-16's per-action risk-assessment discipline applies.
- **Decision:** Three distinct categories, each treated independently.
  - **Synthetic replay** — the caller supplies a synthetic token (their own test string) and PortalLens echoes it back to the portal endpoint to observe validation behavior. **Approvable behind `--authorized`.** Constructive protocol testing; no real credential is involved; access is not obtained. ADR-18's "never attempt to obtain access" is satisfied because the synthetic token has no authorization to extend.
  - **Owned-target replay** — the operator replays a session token they were legitimately issued by the portal, against the portal they own / are authorized to test. **Approvable behind `--authorized` + per-target audit trail.** Access is intentionally to the operator's own authorized target; the risk is bounded exactly to the authorization scope. ADR-18's "never attempt to obtain access" is satisfied because the access being attempted is the access the operator was granted.
  - **Captured-real replay** — replay of a token captured from a third party (a different user, a different legitimate session) against that third party's portal. **Refused.** Operator authorization under `--authorized` attaches to the target machine / network, not to credentials belonging to a captured user. This is exploitation in the sense ADR-16 specifically names as requiring per-action assessment, and the assessment here is "no" because the captured user's consent is absent.
- **Consequences:**
  - The CLI surface (added in a future session, only after this ADR is accepted) exposes `--replay-mode {synthetic, owned}`; `--authorized` is required for either. There is **no** `--replay-mode captured-real` — that path is absent by construction (the parameter's enum excludes it), not merely disabled at runtime.
  - The audit trail records the replay mode alongside the existing evidence. Future reviews can tell synthetic from owned from absence-of-mode.
  - ADR-18's "never attempt to obtain access" stands. ADR-16's per-action discipline is honoured — *this* action has its own assessment, recorded in this ADR.
  - Replays targeting authorization itself (e.g. trying to "extend" an expired session, or grant additional privileges beyond what the token carries) are out of scope and require a future ADR if they are ever desired.
  - If a future ADR lifts this refusal, that ADR must enumerate the conditions under which replay of a captured real session is acceptable (e.g. a red-team engagement for the captured user's employer, with written authorization naming that user). A blanket lift is not consistent with the per-action discipline this decision embodies.
  - **Sibling-required:** the two other per-action review items in `tasks/backlog.md` (default-credential authentication, L2 MAC impersonation) should follow the same shape when their ADRs are written; their respective refusals / acceptances are independent and not bound by this ADR's outcome.

---
## ADR-20: Wi-Fi core starts with credential-free adapter contracts (2026-08-02)
- **Status:** accepted
- **Context:** Live Wi-Fi investigation needs a platform-neutral seam before adding Windows, macOS, and Linux implementations. The host OS should remain responsible for network credentials and association policy, while the TUI needs stable value objects for discovery and connection state.
- **Decision:** Add a dependency-free `portallens.wifi` package containing immutable `WifiNetwork` and `WifiConnection` snapshots, explicit `WifiConnectionState`/`WifiSecurity` enums, `WifiAdapterCapabilities`, typed adapter errors, a pollable `CancellationToken`, and a `WifiAdapter` protocol. The core contract exposes discovery, OS-mediated connection requests, status, disconnect, and cancellation but contains no credential fields and no packet-level operations. Serialization is explicit and allow-listed, so future persistence cannot accidentally include secrets.
- **Alternatives considered:** (A) put Wi-Fi state directly on the TUI — rejected because ADR-8 makes investigation state a core concern; (B) implement platform subprocesses first — rejected because parser/API behavior would become the de facto contract without hardware-independent tests; (C) accept credential mappings in the adapter — rejected because PortalLens must not become a credential store or log sink.
- **Consequences:** Platform adapters can be added independently and tested with fixtures; connection support can be capability-gated; a later event/migration layer can persist these snapshots without changing the adapter API. Mobile-specific restrictions remain a later adapter/capability concern.

---
## ADR-21: Desktop Wi-Fi discovery uses read-only command adapters (2026-08-02)
- **Status:** accepted
- **Context:** The Wi-Fi domain contract needs deterministic desktop implementations before TUI orchestration. Windows, macOS, and Linux expose different host tools and output formats; platform details must not leak into the core models or presentation layer.
- **Decision:** Add an injectable command runner and read-only adapters: structured `netsh` on Windows, `airport` plus `networksetup` on macOS, and NetworkManager `nmcli` on Linux. Parsers normalize each output into `WifiNetwork`/`WifiConnection` values. Commands run without a shell, use bounded timeouts, preserve no raw command output for persistence, and support interruptible cancellation. Concrete adapters advertise scan/status only; association and disconnection are explicitly unsupported and accept no credentials.
- **Consequences:** Fixture tests can cover platform parsing without hardware or subprocesses. Localization differences remain an adapter concern and should be handled with locale-independent output or captured localized fixtures before expanding support. The next slice can build the TUI picker against the stable `WifiAdapter` boundary without importing subprocess logic into `tui/`.

---
## ADR-22: Wi-Fi picker coordinates discovery only (2026-08-02)
- **Status:** accepted
- **Context:** The read-only desktop adapters now provide deterministic scan/status data, and the live console needs a user-facing network picker. The picker must remain a presentation/control surface rather than becoming an association or bypass tool.
- **Decision:** Add a dependency-free `WifiSessionController` for immutable picker state, cancellation, monotonic scan generations, stale-result rejection, and selection validation. Add a lazy Textual `WifiPickerApp` and `portallens wifi` command with scan/rescan/cancel/select controls. Selection ends the slice; it never calls `connect`, accepts credentials, opens a browser, detects a portal, or invokes bypass logic. Keep Textual out of `portallens.wifi` core imports.
- **Consequences:** Rapid rescans cannot replace newer results with stale worker output; stale UI rows are cleared while scanning; callbacks detach safely on unmount. The next slice may attach bounded captive-portal connectivity probes to a selected host-mediated session, but must preserve the picker’s read-only boundary and passive-by-default behavior.

---
## ADR-23: Captive-portal detection is bounded and standards-aware (2026-08-02)
- **Status:** accepted
- **Context:** The read-only Wi-Fi adapters and generation-guarded picker now provide a safe discovery boundary. A selected host-mediated session needs connectivity evidence without opening the browser, following a captive redirect, accepting credentials, or invoking bypass probes. RFC 8908 defines a structured Captive Portal API, while RFC 8910 provisions its endpoint through DHCP/RA.
- **Decision:** Add a bounded `CaptivePortalDetector` with fixed, allow-listed legacy profiles for Windows, Apple, Android, GNOME, and Firefox-style connectivity checks. Disable redirects, cap response bodies, use explicit timeouts, poll `CancellationToken`, and redact URL values before evidence/state handoff. Parse RFC 8908 JSON only when the caller explicitly asserts `provisioned=True` for an HTTPS endpoint obtained from a trusted host/OS source; this boolean is an assertion seam, not DHCP/RA verification. Require `application/captive+json`, validate fields, and never submit credentials or invoke bypass methods. Hand off only confirmed redirection/API portal URLs to the existing passive analyzer.
- **Consequences:** Legacy arbitrary profile URLs cannot turn the detector into an SSRF primitive; RFC 8908 endpoint provenance remains the caller/OS integration responsibility until RFC 8910 option decoding is implemented. Redirects and portal metadata become typed evidence, while generic errors and ambiguous bodies remain unknown. The detector is a library/integration seam only; generation-guarded selected-session orchestration, Textual wiring, and SQLite live-event persistence are deferred to the next slice.

---
## ADR-24: Setup is the front door, but Wi-Fi remains honest until orchestration exists (2026-08-02)
- **Status:** accepted
- **Context:** Users should be able to run `portallens` and configure an investigation visually, while the current desktop adapters expose discovery/status but do not yet connect, detect, persist, or hand off a selected Wi-Fi session.
- **Decision:** Bare `portallens` launches a setup TUI. URL targets hand off to the existing live investigation console. Selected Wi-Fi targets are explicitly labelled and limited to read-only host-status monitoring until selected-session captive detection and live-event persistence are implemented. URL continuous monitoring requires both the active-check choice and explicit authorization; monitoring alone never escalates passive analysis.
- **Consequences:** The primary UX is available now without misrepresenting unsupported network behavior. The next Wi-Fi slice must attach the bounded detector through the generation-guarded session controller, then add redacted event persistence and stop/switch orchestration. Public docs should describe this boundary in user-facing language.

---
## ADR-25: Preserve rendered Wi-Fi rows during selection and refresh (2026-08-02)
- **Status:** accepted
- **Context:** Textual mounts `ListItem` children asynchronously. A selection handler can synchronously publish controller state while Textual is still dispatching `ListView.Selected`; clearing and rebuilding the same list in that callback can crash or leave the list empty. Automatic refresh introduces the same mount race if identical scan results rebuild rows unnecessarily.
- **Decision:** Both Wi-Fi UIs compare a stable, display-relevant row signature (identity, display name, security, and signal) before replacing list children. Selection-only state updates do not mutate the `ListView`. During a scan, existing rows remain visible but the list is disabled until a ready result is rendered. UI scan controls skip requests while a scan is already active; refresh timers are stopped and cleared during unmount.
- **Consequences:** Keyboard selection remains safe, repeated automatic refresh does not duplicate or erase rows, and stale rows cannot be selected during a scan. The controller retains its generation-based supersession API for direct callers; the single-flight guarantee described here applies to the TUI controls. The selected Wi-Fi workflow remains read-only until the separate connection/orchestration slice is implemented.
