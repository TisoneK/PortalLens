...
- **Reason for combined Session-14 deliverables:** the user requested the three Session-14 followups be logged for research *after* the original commit landed. The combined delivery is a single `chore(context): refine session-14 research log per review` commit covering the research-questions.md R1/R2/R3 enrichment, the cross-link, the relocation, this correction, and the SUMMARY.md / notes.md consistency fix.
