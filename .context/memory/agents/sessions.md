...
---
## 2026-08-02 — Session 16
- **Agent:** Buffy | **Model:** deepseek-v4-flash (stated in system prompt) | **Platform:** Tisone's macOS workstation (Darwin 24.6.0 / macOS 15.7.7, local) | **Role:** engineer | **Core:** 0.5.0
- **Task:** User critique: "TUI still feels like non-tui... it's not an advanced hacking tool-like with live updates and controls". Upgrade the static report-viewer TUI into a live investigation console.
- **Commits:** 2 product (40917f6, 6056dfc) + this `chore(context):` log
- **Outcome:** done — TUI rewritten over the persisted `Investigation` aggregate: StatusBar (target, AUTH badge, MODE, live evidence/findings/open-question counters), streaming RichLog activity feed, keyboard controls (1-9 run the computed next-steps from the open questions' "Resolves with" steps; n NetAudit probe pass; m monitor toggle; a auto-run toggle; s save; e export Markdown; r refresh; q quit), single-flight `@work(thread=True)` worker (no wedged busy state), evidence dedupe, monitor mode (set_interval + open-port delta feed), auto-run mode, `--auto`/`--monitor`/`--monitor-interval` CLI flags, auto-save at launch. ADR-7 invariants held: pure control layer, zero analysis logic in `tui/`, engine untouched, vendor-hostname + colour-not-only + responsive tests still pass. 231 tests green (31 TUI, 9 new), ruff + mypy clean. Live expect-driven PTY run verified: render, `1` refusal gate under passive policy, `s`/`e`/`m` all work, Markdown export written. Post-push code review caught three real concurrency bugs — unhandled worker exceptions leave the console wedged busy forever, monitor ticks duplicate evidence, dual exclusive workers race the busy flag — all fixed in 6056dfc with regression tests.
- **Open items:** unchanged — R3 mypy-strict baseline in `tests/` (17 pre-existing errors, Session 13) is the most relevant open item; standing feature backlog untouched (CT-log mining, DisclosureDesk, per-action ADR-16 reviews).
- **Report:** .context/memory/reviews/2026-08-02-review-3.md. Notes: .context/memory/sessions/2026-08-02-16/notes.md
...
- **Commits:** 2 (ec62769 product + context repair pending) — `feat(wifi): add credential-free adapter foundation` and context records
